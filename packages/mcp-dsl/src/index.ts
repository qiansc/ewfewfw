#!/usr/bin/env node
/**
 * @c4a/mcp-dsl - DSL 解析验证 MCP Server
 *
 * 提供 DSL 解析、验证、生成等工具
 *
 * 支持两种 transport 模式:
...
async function startStdioServer() {
  ...
  console.error("c4a-dsl-mcp server started (stdio)");
}
...
async function startHttpServer() {
  ...
  const httpServer = createHttpServer(async (req, res) => {
    ...
    if (url.pathname === "/health" && req.method === "GET") {
      ...
      res.end(JSON.stringify({ status: "ok", service: "c4a-dsl-mcp" }));
      return;
    }
  ...
  httpServer.listen(port, "0.0.0.0", () => {
    console.log(`c4a-dsl-mcp server started (http://0.0.0.0:${port}/mcp)`);
