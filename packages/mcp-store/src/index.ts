#!/usr/bin/env node
/**
 * @c4a/mcp-store - 知识存储 MCP Server
 *
 * 提供 c4a_store_* 工具（保存/读取/同步/feat 管理等）
 *
 * 支持两种 transport 模式:
...
async function startStdioServer() {
  ...
  console.error("c4a-store-mcp server started (stdio)");
}
...
async function startHttpServer() {
  ...
  const httpServer = createHttpServer(async (req, res) => {
    ...
    if (url.pathname === "/health" && req.method === "GET") {
      ...
      res.end(JSON.stringify({ status: "ok", service: "c4a-store-mcp" }));
      return;
    }
  ...
  httpServer.listen(port, "0.0.0.0", () => {
    console.log(`c4a-store-mcp server started (http://0.0.0.0:${port}/mcp)`);
