...
export const menuTree: MenuItem[] = [
  ...
  { id: "build", label: "build", description: "编译 CLI" },
  {
    ...
    description: "调试 MCP",
    children: [
      { id: "debug:store", label: "store", description: "Store MCP" },
      { id: "debug:extract", label: "extract", description: "Extract MCP" },
      { id: "debug:query", label: "query", description: "Query MCP" },
    ],
  },
  {
    id: "server",
...
export const helpDescriptions: Record<string, string> = {
  ...
  "debug:extract": "前台运行 mcp-extract (stdio 模式)，可直接看到输入输出日志，Ctrl+C 退出。",
  "debug:query": "前台运行 mcp-query (stdio 模式)，可直接看到输入输出日志，Ctrl+C 退出。",
