...
export async function generateContract(
  ...
  if (pathStat.isFile()) {
    const analysis = await parseFile(path);
    analysis.file = path;
  ...
  switch (format) {
    case "openapi":
      return generateOpenAPI(analyses, { version, title, description, baseUrl });
    case "asyncapi":
      return generateAsyncAPI(analyses, { version, title, description });
    case "proto":
      return generateProto(analyses, { title });
    default:
...
function generateOpenAPI(
  ...
  const { version = "3.0.0", title = "API", description = "", baseUrl = "/" } = options;
  ...
  for (const analysis of analyses) {
    ...
    allMethods.push(...analysis.functions);
  }
...
function generateAsyncAPI(
  ...
  const { version = "2.6.0", title = "Async API", description = "" } = options;
  ...
  for (const analysis of analyses) {
    allInterfaces.push(...analysis.interfaces);
    allMethods.push(...analysis.functions.filter((m) => m.async));
  }
...
function generateProto(
  ...
  for (const analysis of analyses) {
    allInterfaces.push(...analysis.interfaces.filter((i) => i.exported));
    allMethods.push(...analysis.functions);
  }
