#!/usr/bin/env node
/**
 * @c4a/mcp-query - 知识查询 MCP Server
 *
 * 提供 c4a_query_* 工具（搜索/依赖/影响分析等）
 *
 * 支持两种 transport 模式:
 ...
 * - MCP_PORT: HTTP 端口 (默认 8054)
 */
import { createServer } from "./server.js";

const transport = process.env.MCP_TRANSPORT || "stdio";
const port = parseInt(process.env.MCP_PORT || "8054", 10);
...
async function startStdioServer() {
  ...
  console.error("c4a-query-mcp server started (stdio)");
}
...
async function startHttpServer() {
  ...
  const httpServer = createHttpServer(async (req, res) => {
    ...
    if (url.pathname === "/health" && req.method === "GET") {
      ...
      res.end(JSON.stringify({ status: "ok", service: "c4a-query-mcp" }));
      return;
    }
  ...
  httpServer.listen(port, "0.0.0.0", () => {
    console.log(`c4a-query-mcp server started (http://0.0.0.0:${port}/mcp)`);
